...
export default function Login() {
  ...
  return (
    <Box component='section' className='login-form' sx={{ textTransform: 'uppercase' }}>
      <BackHome  />
      <Typography component='h1' variant='h1'>{t('Sign in to Access Your Account')}</Typography>
      ...
      <Box component={'form'} onSubmit={handleForgotPassword} marginBottom='80px' >
        <Box component={'button'} type={'submit'} border={'none'} sx={{ background: 'none', textTransform: 'uppercase' }}>
          <LineHover text={t('Forgot your password?')} />
        </Box>
      </Box>
</Box>
      <Box sx={{ textTransform: 'uppercase', maxWidth:{xs:'50%',sm:'50%',md:'15%' }}}>
        <Typography variant='h3' fontWeight={'700'} marginBottom={'16px'} sx={{}}>{t('CREATE AN ACCOUNT NOW')}</Typography>
        <Box component={routerLink} to="/Register" sx={{ textDecorationLine: 'none' }}>
          <Registeration text={t('CREATE ACCOUNT')} />
        </Box>
